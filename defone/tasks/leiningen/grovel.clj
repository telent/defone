...
(defn grovel-specs [record-specs]
...
(defn grovel [project output-file]
  "Grovel constants from C header files"
  (let [f (io/file output-file)
        defns (grovel-specs @types-to-grovel)]
    (-> f .getParentFile .mkdirs)
    (binding [*out* (io/writer f)]
      (prn '(ns defone.structs))
      (prn `(def defone.structs/grovelled ~defns)))))
