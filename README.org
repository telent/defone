...
There is an actual clojure project on the device in /defone/defone/
...
This is a bit sucky having everything in one clojure project because
it means bringing the whole platform down whenever we want to add
another library - or whever we get the jvm into a bad state and need
to restart this.  Definitely a topic we will need to revisit
